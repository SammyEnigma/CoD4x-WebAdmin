...
module.exports = {
	...
	getLogin: function(req, res, next) {
		...
		.then (function(results){
			res.render('auth/login.pug', { title: 'Login', results: results, csrfToken: req.csrfToken(), message: req.flash('loginMessage') });
		}).catch (function(err){
			console.log(err);
			res.redirect('back');
		});
	...
	getSignup: function(req, res, next) {
		Plugins.findAsync({'category' : 'sso', 'status':true})
		.then (function(results){
		res.render('auth/signup.pug', { title: 'Sign Up', results: results, csrfToken: req.csrfToken(), captcha:recaptcha.render(), message: req.flash('signupMessage') });
	}).catch (function(err){
			console.log(err);
			res.redirect('back');
		});
	...
	getProfile: function(req, res, next) {
		BluebirdPromise.props({
			...
			res.render('auth/profile.pug', {title: 'My Profile', results: results, csrfToken: req.csrfToken()});
		}).catch (function(err){
			console.log(err);
	...
	ScreenshotNotificationUpdate: function(req, res, next) {
		User.findOneAndUpdate({'_id':req.user._id,  "local.user_role": {$gt:1}}, {$set:{'account_settings.enable_screenshot_notification':req.body.enable_screenshot_notification, 'account_settings.enable_screenshot_notification_sound':req.body.enable_screenshot_notification_sound}}, {new: true}, function(err, doc){
		    if(err){
		        req.flash('error_messages', 'Only Server Admins can use this feature');
		    }
		    req.flash('success_messages', 'Screenshot Notification changes saved');
		    res.redirect('/user/profile');
		});
	},
	...
	uploadAvatar:  function(req, res, next) {
	    ...
	    .then (function(logo){
			...
			var upload = multer({
				...
				fileFilter: function (req, file, callback) {
			        ...
			        if(ext !== '.png' && ext !== '.jpg' && ext !== '.jpeg') {
			        	req.fileValidationError = 'Only images in JPG and PNG format are allowed';
			            //return callback(new Error('Only images in JPG and PNG formats are allowed'));
			            req.flash('error_messages', 'Only images in JPG and PNG format are allowed');
			            res.redirect('back');
			        }
			        callback(null, true);
			    },
			...
			req.flash('success_messages', 'Avatar successfully changed');
		        res.redirect('back');
				}
	...
	RemoveAvatar: function(req, res, next) {
		...
		.then (function(logo){
			...
			req.flash('error_messages', 'Avatar successfully deleted');
		        res.redirect('back');
		}).catch (function(err){
			console.log(err);
			res.redirect('back');
