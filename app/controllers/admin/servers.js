...
module.exports = {
	...
	CreateNewLocalServer: function(req, res, next) {
		...
		var newServer = new Servers ({
			...
			script_starter: './cod4x18_dedrun +set net_ip '+config.ssh_access.host+' +set net_port '+serverport+' +set sv_maxclients '+serverslots+' +set fs_homepath '+homedir+' +set fs_basepath '+homedir+' +set sv_punkbuster 0 +set rcon_password '+req.body.rcon_password+' +exec '+configfile+' +map_rotate +map mp_crash',
			is_online: false,
			is_stoped: true
		});
		newServer.saveAsync()
	...
	ServerRemove: function(req, res, next) {
		BluebirdPromise.props({
			...
			Adminactions.find({ 'rcon_server': req.params.id }).remove().exec();
			Bans.find({ 'rcon_server': req.params.id }).remove().exec();
			Cheaterreports.find({ 'rcon_server': req.params.id }).remove().exec();
			ServerScreenshots.find({ 'get_server': req.params.id }).remove().exec();
			OnlinePlayers.find({ 'server_alias': results.servers.name_alias }).remove().exec();
			//Remove admins from this server
			User.update({'local.admin_on_servers':req.params.id},{$pull:{'local.admin_on_servers':req.params.id}},function(err){
				console.log(err);
			});
		}).then(function(laststep){
	  		Servers.findOne({ '_id': req.params.id }).remove().exec();
	...
	RconConsoleRemove: function(req, res) {
		Rconconsole.remove({'rcon_server': req.params.id})
...
function includecod4authtoken(string, plus_string) {
	string = S(string).replaceAll('server.cfg', plus_string).s;
	return string;
}
